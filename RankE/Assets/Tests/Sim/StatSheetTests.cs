...
namespace RankE.Sim.Tests
{
    ...
    public class StatSheetTests
    {
        ...
        {
            var a = TestKit.Config("A", DefaultContent.Slash());
            a.Stats.Attack = 50;
        ...
        public void Defense_ReducesIncomingDamage()
        {
            ...
            var b = TestKit.Duel(TestKit.Config("A", DefaultContent.Slash()), def);
            TestKit.UseAt(b, 0, 0, DefaultContent.SlashId);
        ...
        {
            var cfg = TestKit.Config("A", DefaultContent.Slash(), DefaultContent.Fireball());
            cfg.Stats.Haste = 50;
            var f = new Fighter(0, cfg, DefaultContent.CreateContent());
            Assert.AreEqual((int)(DefaultContent.Slash().CooldownTicks * 0.5), f.GetAbility(DefaultContent.SlashId).EffCooldownTicks); // base × 0.5
            Assert.AreEqual(20, f.GetAbility(DefaultContent.FireballId).EffCastTicks);  // int(40 × 0.5)
        }

        [Test]
        public void GemRegen_RestoresAfterSpend_CapsAtMax()
        {
            var cfg = TestKit.Config("A", DefaultContent.Fireball());
            cfg.Stats.GemRegenIntervalTicks = 20;
            var b = TestKit.Duel(cfg, TestKit.Config("B"));

            TestKit.UseAt(b, 0, 0, DefaultContent.FireballId); // cast; consumes 1 gem on completion
